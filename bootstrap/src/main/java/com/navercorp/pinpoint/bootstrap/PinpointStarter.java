...
import com.navercorp.pinpoint.bootstrap.config.AicaiProfilerConfig;
...
class PinpointStarter {
    ...
    boolean start() {
        ...
        //add.lijian.to fetch config from config center
        ProfilerConfig profilerConfig =null;
        try {
            profilerConfig = AicaiProfilerConfig.remoteLoad(applicationName, "pinpoint-config", configPath);

            if (profilerConfig==null)
            {
                logger.warn("pinpoint agent start failed!");
                return false;
            }

            boolean load_option = profilerConfig.getProfilerLoadable();
            if (!load_option) {
                logger.warn("pinpoint agent attach disabled!");
                return false;
            }
            else
            {
                logger.warn("pinpoint agent attach enabled!");
            }
        } catch (Exception e) {
            logger.warn(ProductInfo.NAME + " start failed.", e);
            return false;
        }
         //end.lijian.remove to upper side to load class of configure center
        ...
        try {
            ...
            logger.info("libs loaded as" + libUrlList.toString());
