 ...
 * @author netspider
 */
public class AicaiProfilerConfig implements ProfilerConfig {

    //add.lijian.20180126
    private static final Properties diamondProperties = new Properties();
    //end.lijian
    ...
    //add.lijian.20180126
    public static ProfilerConfig remoteLoad(String groupId, String dataId, String pinpointConfigFileName) throws Exception 
    {

        final CountDownLatch ok_signal = new CountDownLatch(1);
        ...
        try
        {
            DiamondManager manager = new DefaultDiamondManager(groupId,dataId, new ManagerListener() {
    			...
    			public void receiveConfigInfo(String configInfo) {
                    ...
                    System.out.print("configure info is: \n" + configInfo + "\n");
    				AicaiProfilerConfig.diamondProperties.putAll(getPropertiesForString(configInfo));  
            ...
            boolean reachOK = ok_signal.await(50000, TimeUnit.MILLISECONDS);
            
            
            if (!reachOK)
            {
                System.out.print("time emmit, load configure file! \n");
                return load(pinpointConfigFileName);
            }

            System.out.print("fetch configuration from configure center Ander!\n");
            System.out.print("\n" + AicaiProfilerConfig.diamondProperties.toString() + "\n");

            return new AicaiProfilerConfig(AicaiProfilerConfig.diamondProperties);

        }
        catch(Exception e)
        {
            if (logger.isWarnEnabled()) {
                logger.warn(" Diamond Config Center pull configuration error. Error:" + e.getMessage(), e);
            }
            throw e;
        }    
    ...
    private static Properties  getPropertiesForString(String configinfo)
    {
    	...
    	{
    		String kvline = line.trim();
    		if (kvline.length()==0) continue;
    		if (kvline.startsWith("#")) continue;	
    		String[] kvs = kvline.split("=");
    		if (kvs.length<2) continue;
    		pros.put(kvs[0].trim(), kvs[1].trim());
    	}
    ...
    //add.lijian.20180126
    private boolean profilerLoadable = false;
    //end.lijian
    ...
    public AicaiProfilerConfig() {
        this.properties = new Properties();
    }

    public AicaiProfilerConfig(Properties properties) {
        if (properties == null) {
    ...
    //add.lijian.20180126
    @Override
    public boolean getProfilerLoadable()
    {
        return this.profilerLoadable;
    }
    //end.lijian
    ...
    void readPropertyValues() {
        ...
        //add.lijian.20180126
        this.profilerLoadable = readBoolean("profiler.loadable", false);
        //end.lijian
